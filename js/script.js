const FriendManager = (() => {
    ...
    const fetchNonFriends = async () => {
      try {
        const response = await fetch("http://127.0.0.1:8000/api/fetch_non_friends/", {
          method: "GET",
          headers: {
    ...
    const fetchFriendRequests = async () => {
        try {
          const response = await fetch("http://127.0.0.1:8000/list_requst_friend/", {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
            },
    ...
    const acceptFriendRequest = async username => {
      try {
        const response = await fetch("http://127.0.0.1:8000/accept_friend_request/", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    const removeFriend = async username => {
      try {
        const response = await fetch("http://127.0.0.1:8000/remove_friend/", {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    const renderList = dataList => {
      if (dataList.length === 0) {
        userListContainer.innerHTML = `
                  <h2>${currentMode === "users" ? "List of Users" : "Friend Requests"}</h2>
                  <p style="text-align: center; color: gray;">${currentMode === "users" ? "No users found." : "No friend requests."}</p>
              `;
        return;
      }
      ...
      userListContainer.innerHTML = `
              ...
              ${dataList.map(data => `
                  <div class="user">
                      <img src="${data.image}" alt="${data.name}" class="user-image">
                      <p>${data.name}</p>
                      ${currentMode === "users" ? `
                          <button class="action-btn user-btn" data-username="${data.name}" 
                              style="display: ${data.invitation_sent ? 'none' : 'inline-block'};">
                              Invite
                          </button>
                      ` : ""}
    ...
    const initialize = async () => {
      ...
      searchInput.addEventListener("input", event => {
        ...
        const filteredList = dataList.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase()));
